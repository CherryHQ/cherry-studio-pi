...
export type ModelTokenLimitFields = Pick<Model, 'context_window' | 'max_input_tokens' | 'max_output_tokens'>
...
export function resolveModelTokenLimits(model?: Model): ModelTokenLimits {
  const metadataContextWindow = positiveTokenLimit(model?.context_window)
  const maxInputTokens = positiveTokenLimit(model?.max_input_tokens)
  const metadataMaxOutputTokens = positiveTokenLimit(model?.max_output_tokens)
