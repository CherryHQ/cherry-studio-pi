...
function getUniqueAssistants(): LegacyAssistant[] {
  const state = store.getState()
...
export function upsertStorageV2File(file: FileMetadata): Promise<{ imported: boolean; skippedReason?: string }> {
  return window.api.storageV2.upsertFile(file as unknown as Record<string, unknown>)
}
...
export async function prepareStorageV2ForDataSync() {
  await flushStorageV2RuntimeMirrors()
