...
import type { LegacyAssistant as Assistant } from '@renderer/types'
...
function getAssistantSettings(assistant: Assistant) {
  return {
...
function getAssistantModel(assistant: Assistant) {
  return assistant.model ?? assistant.defaultModel ?? assistant.settings?.defaultModel
}
...
export async function applyChatContextBudget(
  ...
  if (messages.length === 0) {
    const limits = resolveModelTokenLimits(getAssistantModel(assistant))
  ...
  if (stats.removed > 0) {
    logger.debug('Applied chat context budget', {
      ...
      outputReserve,
      safetyBuffer,
      modelId: getAssistantModel(assistant)?.id,
