...
export function resolveCreateSessionAgentId(
  sessions: readonly Pick<AgentSessionEntity, 'id' | 'agentId'>[],
...
const Sessions = ({ onSelectItem }: SessionsProps) => {
  ...
  // default, then the first listed session, then the first available agent.
  // The final fallback keeps "has agents but no sessions yet" recoverable.
  ...
  if (error) {
    return (
      <div
        role="alert"
        className="m-2.5 flex items-start gap-2 rounded-md border border-destructive bg-destructive/10 px-3 py-2 text-sm">
        <AlertTriangle className="mt-0.5 size-4 shrink-0 text-destructive" />
        <div className="flex flex-1 flex-col gap-1">
          <div className="font-medium">{t('agent.session.get.error.failed')}</div>
          <div className="text-foreground-muted text-xs">{formatErrorMessage(error)}</div>
          <div>
