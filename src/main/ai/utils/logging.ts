...
export function summarizeAssistantForLog(assistant: Assistant) {
  const legacyAssistant = assistant as Assistant & {
    type?: string
    mcpServers?: unknown[]
  ...
  return {
    id: assistant.id,
    name: assistant.name,
    type: legacyAssistant.type,
    hasPrompt: Boolean(assistant.prompt),
    settingsKeys: Object.keys(assistant.settings ?? {}),
    mcpServerCount: legacyAssistant.mcpServers?.length ?? assistant.mcpServerIds?.length ?? 0,
    knowledgeBaseCount: legacyAssistant.knowledge_bases?.length ?? assistant.knowledgeBaseIds?.length ?? 0,
    hasModel: Boolean(legacyAssistant.model ?? assistant.modelId)
  }
}
