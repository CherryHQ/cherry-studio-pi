...
function getImageHandlers(service: InstanceType<typeof AiService>) {
  return {
    generateImage: (_event: unknown, request: { requestId: string; payload: Record<string, unknown> }) =>
      service.runImageRequest(request.requestId, request.payload as never),
    abortImage: (_event: unknown, request: { requestId: string }) => service.abortImage(request.requestId)
  }
}
