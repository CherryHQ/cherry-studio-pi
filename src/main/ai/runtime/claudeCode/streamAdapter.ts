...
type SDKInitSystemMessage = Extract<SDKMessage, { type: 'system'; subtype: 'init' }>
type SDKThinkingTokensMessage = Extract<SDKMessage, { type: 'system' }> & {
...
export class ClaudeCodeStreamAdapter {
  ...
  private handleSystemMessage(message: Extract<SDKMessage, { type: 'system' }>, ctx: StreamContext): void {
    ...
    switch (subtype) {
      case 'init':
        this.handleInitSystemMessage(message as SDKInitSystemMessage, ctx)
        return
      case 'task_started':
      case 'task_progress':
      case 'task_updated':
      case 'task_notification':
        this.handleTaskSystemMessage(message as SDKTaskSystemMessage, ctx)
        return
      case 'status':
        this.handleStatusSystemMessage(message as SDKStatusMessage)
        return
      case 'compact_boundary':
        this.handleCompactBoundarySystemMessage()
        return
      case 'thinking_tokens':
        this.handleThinkingTokensSystemMessage(message as SDKThinkingTokensMessage, ctx)
        return
      case 'api_retry':
      case 'hook_started':
      case 'hook_progress':
  ...
  private handleInitSystemMessage(message: SDKInitSystemMessage, ctx: StreamContext): void {
