...
import type { AgentRuntimeUserInput } from '@main/ai/runtime/types'
...
import { getPathStatus } from '@main/utils/file/pathStatus'
...
type AgentSessionWithWorkspace = AgentSessionEntity & {
  workspace: NonNullable<AgentSessionEntity['workspace']>
}
...
async function ensureManagedSystemWorkspaceDirectory(cwd: string): Promise<void> {
  ...
  if (!isPathInsideOrEqual(resolvedCwd, resolvedRoot)) {
    throw new AgentSessionWorkspaceError(
      t('agent.session.workspace_status.inaccessible', {
        path: cwd
      })
    )
  }
...
export async function buildMcpServers(
  ...
  if (soulEnabled) {
    ...
    const workspaceSession = session as AgentSessionWithWorkspace
    const sourceChannelId = await resolveSourceChannel(agent.id, session.id)
    let workspaceSource: AgentSessionWorkspaceSource
    switch (workspaceSession.workspace.type) {
      case AGENT_WORKSPACE_TYPE.USER:
        workspaceSource = { type: AGENT_WORKSPACE_TYPE.USER, workspaceId: workspaceSession.workspace.id }
        break
      case AGENT_WORKSPACE_TYPE.SYSTEM:
        workspaceSource = { type: AGENT_WORKSPACE_TYPE.SYSTEM }
        break
      default: {
        const exhaustive: never = workspaceSession.workspace.type
    ...
    const memoryServer = new WorkspaceMemoryServer(agent.id, workspaceSession.workspace.path)
