...
import {
  ...
  missingAgentSessionWorkspaceError
} from '../agentSessionWorkspace'
...
async function reconcileSkillMirror(agentId: string, cwd: string): Promise<void> {
  const service = skillService as typeof skillService & {
    reconcileAgentSkills?: (agentId: string, cwd: string) => Promise<void>
    reconcileSkills?: () => Promise<void>
  }

  if (typeof service.reconcileSkills === 'function') {
    await service.reconcileSkills()
    return
  }

  await service.reconcileAgentSkills?.(agentId, cwd)
}
...
export async function buildClaudeCodeSessionSettings(
  ...
  await reconcileSkillMirror(session.agentId, cwd)
...
export async function prepareClaudeCodeWorkspaceDirectory(session: AgentSessionEntity): Promise<void> {
  ...
  if (session.workspace.type === AGENT_WORKSPACE_TYPE.SYSTEM) {
    await ensureManagedSystemWorkspaceDirectory(session.workspace.path)
  }
  await assertClaudeCodeWorkspaceDirectory(session.id, session.workspace.path)
}
...
async function ensureManagedSystemWorkspaceDirectory(cwd: string): Promise<void> {
  const managedRoot = application.getPath('feature.agents.workspaces')
  const resolvedRoot = path.resolve(managedRoot)
  const resolvedCwd = path.resolve(cwd)

  if (!isPathInsideOrEqual(resolvedCwd, resolvedRoot)) {
    throw new AgentSessionWorkspaceError(t('agent.session.workspace_status.inaccessible', { path: cwd }))
  }
  ...
  try {
    ...
    // The managed root is created lazily together with the session workspace.
  }

  try {
    const realCwd = await fs.promises.realpath(resolvedCwd)
    if (!isPathInsideOrEqual(realCwd, realRoot)) {
      throw new AgentSessionWorkspaceError(t('agent.session.workspace_status.inaccessible', { path: cwd }))
    }
  } catch (error) {
    if (error instanceof AgentSessionWorkspaceError) throw error
  ...
  const status = await getPathStatus(resolvedCwd)
...
async function buildEnvironment(provider: Provider, agent: AgentEntity): Promise<Record<string, string | undefined>> {
  ...
  // Resolve plan/small models independently: a missing optional model must fall back to its own raw
  // id, while the main model still uses the stricter runtime resolver that handles legacy/api ids.
  const resolveOptionalApiModelId = async (storedModelId: string | null | undefined): Promise<string> => {
    const trimmedModelId = storedModelId?.trim()
    if (!trimmedModelId) return apiModelId

    try {
      const resolvedModel = await resolveAgentRuntimeModel({ id: agent.id, model: trimmedModelId })
      return getAgentRuntimeApiModelId(resolvedModel)
    } catch {
      return isUniqueModelId(trimmedModelId) ? parseUniqueModelId(trimmedModelId).modelId : trimmedModelId
    }
  }
  const sonnetApiModelId = await resolveOptionalApiModelId(agent?.planModel ?? agent.model)
  const haikuApiModelId = await resolveOptionalApiModelId(agent?.smallModel ?? agent.model)
...
export async function buildSkillWhitelist(agentId: string, cwd: string): Promise<string[]> {
  const service = skillService as typeof skillService & {
    list?: (filter: { agentId: string }) => Promise<Array<{ folderName: string; isEnabled: boolean }>>
    listLocal?: (cwd: string) => Promise<Array<{ filename: string }>>
  }

  if (typeof service.list !== 'function' || typeof service.listLocal !== 'function') {
    return []
  }

  const installedSkills = await service.list({ agentId })
  const enabledNames = installedSkills.filter((skill) => skill.isEnabled).map((skill) => skill.folderName)

  const workspaceSkills = await service.listLocal(cwd)
  ...
  if (soulEnabled) {
    ...
    switch (workspace.type) {
      case AGENT_WORKSPACE_TYPE.USER:
        workspaceSource = { type: AGENT_WORKSPACE_TYPE.USER, workspaceId: workspace.id }
        break
    ...
    const clawServer = new ClawServer(agent.id, workspaceSource, workspace.path, sourceChannelId)
    ...
    const memoryServer = new WorkspaceMemoryServer(agent.id, workspace.path)
