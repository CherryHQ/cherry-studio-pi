 ...
 */
if (isLinux) {
  app.commandLine.appendSwitch('class', 'CherryStudioPi')
  app.commandLine.appendSwitch('name', 'CherryStudioPi')
}
...
if (!app.requestSingleInstanceLock()) {
  ...
  void app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(import.meta.env.VITE_MAIN_BUNDLE_ID || 'com.cherryai.cherrystudio-pi')
    ...
    void runAsyncFunction(async () => {
      ...
      // Start the external API server only when the user explicitly enables it.
      // The in-app Agent UI uses IPC now and does not need this HTTP server.
      try {
