...
/**
 * Get Git Bash path info for dependency status reporting.
 *
 * Git Bash no longer has a persisted settings surface in this branch; discovery
 * is runtime-only, with CLAUDE_CODE_GIT_BASH_PATH as the manual override.
 */
export function getGitBashPathInfo(): GitBashPathInfo {
  ...
  const envOverride = process.env.CLAUDE_CODE_GIT_BASH_PATH
  if (envOverride && validateGitBashPath(envOverride)) {
    return { path: path.resolve(envOverride), source: 'manual' }
  }

  const discoveredPath = autoDiscoverGitBash()
  return { path: discoveredPath, source: discoveredPath ? 'auto' : null }
}
