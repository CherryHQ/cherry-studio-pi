...
const MANAGED_RUNTIME_DISABLED_ENV = 'CHERRY_STUDIO_PI_MANAGED_RUNTIME_DISABLED'
...
export function isManagedRuntimeDisabled(): boolean {
  return process.env[MANAGED_RUNTIME_DISABLED_ENV] === '1'
}

export function disableManagedRuntime(): void {
  process.env[MANAGED_RUNTIME_DISABLED_ENV] = '1'
}

export function enableManagedRuntime(): void {
  delete process.env[MANAGED_RUNTIME_DISABLED_ENV]
}

export async function extractRtkBinaries(): Promise<void> {
  await fs.promises.mkdir(getUserBinDir(), { recursive: true }).catch(() => undefined)
}
