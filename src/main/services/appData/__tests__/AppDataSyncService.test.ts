...
密码：test-webdav-password`,
  ...
  it('does not prefer remote Storage v2 records after the same sync space was already joined', async () => {
    ...
    expect(mocks.runtimeProjection.projectProviders.mock.calls[0][0].apiKeyProviderIds).toEqual(new Set())
  ...
  it('projects provider runtime when synced Storage v2 records only contain an api key tombstone', async () => {
    const credentialEntityId = JSON.stringify(['openai', 'apiKeys'])
    const existingStorageManifest = {
      version: 1,
      records: {
        [`sync_tombstone:provider_credential:${credentialEntityId}`]: {
          entityType: 'sync_tombstone',
          table: 'sync_tombstones',
          idValues: ['provider_credential', credentialEntityId],
          valueHash: 'provider-credential-tombstone-hash',
          updatedAt: 1760000000000,
          deletedAt: null,
          version: 1,
          path: 'storage-v2/records/sync_tombstone/provider-credential-apiKeys.json'
        }
      },
      blobs: {}
    }
