...
describe('StorageV2WebDavRecordSyncService', () => {
  ...
  it('does not publish provider rows that still contain plaintext credential fields', async () => {
    const remote = makeSharedWebDavStore()
    const db = makeProviderDb([
      {
        ...
        config_json: JSON.stringify({
          ...
          type: 'openai',
          name: 'OpenAI',
          apiKeys: [{ id: 'primary', key: 'sk-plaintext', isEnabled: true }],
          authConfig: {
            type: 'oauth',
  ...
  it('fails safe when the remote Storage v2 bundle manifest is missing its hash', async () => {
    const remote = makeSharedWebDavStore()
    const bundlePath = '/remote-root/sync/v1/storage-v2/bundle/no-hash.json'
    remote.files.set(
      bundlePath,
      JSON.stringify({
        version: 1,
    ...
    await expect(
      new StorageV2WebDavRecordSyncService([settingsTable]).sync(remote.client as any, '/remote-root/sync/v1', {
        ...
        bundle: {
          version: 1,
          path: 'storage-v2/bundle/no-hash.json',
          recordCount: 0,
          blobCount: 0,
          updatedAt: Date.parse('2026-05-29T12:00:00.000Z')
        } as any
      })
    ).rejects.toThrow('远端 Storage v2 bundle manifest 格式损坏')
  ...
  it('fails safe when the remote Storage v2 records manifest uses Windows-style paths', async () => {
    const remote = makeSharedWebDavStore()
      ...
      new StorageV2WebDavRecordSyncService([settingsTable]).sync(remote.client as any, '/remote-root/sync/v1', {
        ...
        records: {
          'settings:theme': {
            ...
            deletedAt: null,
            version: 1,
            path: 'storage-v2\\records\\settings\\theme.json'
          }
        },
        blobs: {},
        bundle: null
  ...
  it('rejects remote Storage v2 blob storage paths that would escape the data root', async () => {
    const blobRow: BlobRow = {
      id: 'blob-1',
    ...
    vi.mocked(storageV2Database.getClient).mockResolvedValueOnce(db.client as any)
    mocks.remoteFiles.set(recordPath, JSON.stringify(remoteRecord))
    mocks.remoteFiles.set(blobPath, Buffer.from('blob'))
    ...
    ).rejects.toThrow('Storage v2 blob path is invalid')
  ...
  it('downloads remote Storage v2 records that appear after a local first-join defer', async () => {
    const remote = makeSharedWebDavStore()
    const localRow = {
      key: 'theme',
      value_json: '{"mode":"local-default"}',
    ...
    const db = makeSettingsDb([localRow])
    db.state.syncState.set('webdav-storage-record:settings:theme:hash', `deferred-local:${localHash}`)
    vi.mocked(storageV2Database.getClient).mockResolvedValueOnce(db.client as any)
    ...
    expect(result.manifest.records['settings:theme']?.valueHash).toBe(remoteHash)
    expect(result.syncStates).toEqual(
      expect.arrayContaining([
        {
          id: 'settings:theme',
    ...
    expect(Array.from(remote.files.values()).some((contents) => String(contents).includes('local-default'))).toBe(false)
  })
