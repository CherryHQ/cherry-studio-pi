...
describe('StorageV2Service', () => {
  ...
  it('stores sensitive providerSettings extra headers without dropping them from provider metadata', async () => {
    const provider = {
      id: 'provider-1',
      name: 'OpenAI',
      presetProviderId: 'openai',
      ...
      providerSettings: {
        extraHeaders: {
          Authorization: 'Bearer provider-settings-secret',
          'X-Provider-Trace': 'provider-trace-id'
        }
      }
    } as unknown as Provider
    mocks.secretVault.setSecret.mockResolvedValue('storage-v2://secret/provider/provider-1/extraHeaders')
  ...
  it('only clears provider credentials through explicit clear options', async () => {
    const provider: Provider = {
      id: 'provider-1',
      type: 'openai',
      name: 'OpenAI',
    ...
    await expect(
      new StorageV2Service().upsertProvider(provider, 2, undefined, { clearCredential: true })
    ).resolves.toEqual({ skippedSecret: false })
    expect(mocks.providerRepository.upsert).toHaveBeenLastCalledWith(provider, 2, undefined, {
  ...
  it('keeps runtime api keys unchanged when synced provider credentials cannot be restored', async () => {
    mocks.providerRepository.list.mockResolvedValue([
      {
        id: 'openai',
        type: 'openai',
  ...
  it('restores provider api keys from the standard vault ref when credential rows were lost', async () => {
    mocks.providerRepository.list.mockResolvedValue([
      {
        id: 'openai',
        name: 'OpenAI',
    ...
    mocks.providerRepository.listCredentialRefs.mockResolvedValue(new Map())
    mocks.secretVault.getSecret.mockImplementation(async (secretRef: string) => {
      if (secretRef === 'storage-v2://secret/provider/openai/apiKey') return 'sk-restored'
      return null
    })
  ...
  it('restores scoped provider extra headers from credential refs in core snapshots', async () => {
    mocks.providerRepository.list.mockResolvedValue([
      {
        id: 'openai',
        name: 'OpenAI',
    ...
    mocks.secretVault.getSecret.mockImplementation(async (secretRef: string) => {
      if (secretRef === 'storage-v2://secret/provider/openai/extraHeaders') {
        return JSON.stringify({
          settings: {
            Authorization: 'Bearer settings-restored'
          },
          providerSettings: {
            Cookie: 'provider-settings-cookie'
          }
        })
      }
      return null
    ...
    const snapshot = await new StorageV2Service().getCoreSnapshot({ includeSecrets: true })
    const providers = snapshot.llm.providers as Array<Record<string, any>>
    ...
    expect(providers[0].providerSettings).toEqual({
      extraHeaders: {
        'X-Provider-Trace': 'provider-trace-id',
        Cookie: 'provider-settings-cookie'
      }
