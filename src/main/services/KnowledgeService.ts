  ...
  private preprocessing = async (
    ...
    if (base.preprocessProvider && file.ext.toLowerCase() === '.pdf') {
      try {
        ...
        if (alreadyProcessed) {
          logger.debug('File already preprocess processed, using cached result', {
        ...
        logger.debug('Starting preprocess processing for scanned PDF', { filePath: summarizeTextForLog(filePath) })
