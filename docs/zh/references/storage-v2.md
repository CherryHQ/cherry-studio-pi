...
带 `secret_ref` 的远端 Storage v2 记录必须先解密远端 secret bundle、确认所有引用的 secret id 存在，并把对应 secret 导入本机后，才允许写入本地数据库；否则 provider、模型、channel 或设置可能显示已同步但缺少真正凭据。
...
- 当前 `main.db` / `manifest.json` / `blobs` / `secrets` / `KnowledgeBase` / `Memory` / `Skills` / `Agents` / `Workbench` 会先归档到 `legacy/pre-restore-*`，再从备份目录恢复。`Memory/memories.db` 备份使用 `VACUUM INTO`，避免直接复制正在写入的 SQLite DB。
...
- 模型 provider 新增、删除、排序、字段更新、模型列表更新、Vertex / AWS Bedrock 敏感配置更新，以及默认 / quick / 翻译模型切换后会主动 flush Redux -> Storage v2 mirror，降低 API key、服务端地址、service account private key、默认模型这类关键设置在防抖窗口内丢失的风险；provider 模型列表更新时，缺失模型会写 `deleted_at` 软删除，而不是物理删除 `models` 行；仍存在或重新出现的模型会走 upsert 并清空 `deleted_at`，避免重复保存 provider 时主键冲突，也避免自定义模型删除证据在未来同步/恢复中丢失。自定义模型单个删除和批量删除会先通过 partial `llm` snapshot 写入下一版 providers/model 列表，成功后才更新 Redux runtime cache 并 strict flush，避免批量删除时旧模型在恢复中复活。
- assistant 新增、复制、插入、删除、排序、编辑、默认助手更新、模型和 settings 更新后会主动 flush Redux -> Storage v2 mirror；assistant topic 消息仍由普通对话 mirror 负责，核心 assistant 元数据不会只依赖低频防抖写入。provider 和 assistant 删除会先走 Storage v2 direct tombstone IPC，成功后才更新 Redux runtime cache 并 strict flush；MCP server 删除会先通过 partial Redux import 写入下一版 `redux.mcp` 并复用 secret vault 清洗逻辑，assistant preset 单个删除和批量删除会先写入下一版 `assistants.presets`，成功后才更新 Redux runtime cache；assistant topic 单个删除、批量删除和清空会先写入下一版 `assistants` topic 列表，成功后才更新 Redux runtime cache 并 strict flush；topic 移动会先更新 Dexie 消息归属，再写入来源/目标助手的下一版 topic 列表，避免已删除或已移动 topic 在恢复时重新挂回旧助手。
- 网页搜索 provider/订阅源/压缩设置、文档预处理 provider/default provider、OCR provider/image provider/config 写入后也会主动 flush Redux -> Storage v2 mirror；这些配置可能包含 API key、basic auth、OCR/预处理服务端地址，不能只等待低频防抖。网页搜索订阅源删除或整表替换会先通过 partial Redux import 写入下一版 `redux.websearch`，OCR provider 删除会先写入下一版 `redux.ocr`，成功后才更新 Redux runtime cache 并 strict flush，避免订阅源列表、OCR API key 或服务端配置在恢复后回退。
- code tools 的 CLI、模型、终端、环境变量和当前目录切换会主动 flush Redux -> Storage v2 mirror；删除目录、清空目录和重置 code tools 设置会先通过 partial Redux import 写入下一版 `redux.codeTools`，成功后才更新 Redux runtime cache 并 strict flush，避免本地工作区和环境变量配置在恢复时回退。
- Copilot 状态 reset 和快捷键 reset 会先通过 partial Redux import 写入下一版 `redux.copilot` / `redux.shortcuts`，成功后才更新 Redux runtime cache，避免敏感 headers 或快捷键配置在恢复后回退。
- Storage v2 Redux mirror 还会对所有持久化 Redux 配置 slice 跳过 1.2 秒防抖并立即 flush；普通聊天流仍由 conversation mirror 负责，避免高频消息把 settings 表写爆，同时兜住设置页直接 dispatch 的路径。画作历史删除会先通过 partial Redux import 写入下一版 `redux.paintings`，成功后才更新 Redux runtime cache，并在 strict flush 成功后清理相关文件，避免图片历史恢复后指向已经 tombstone 的文件；重新生成画作时，旧文件会等新 files 写入画作记录并 strict flush 后再清理，生成失败或 mirror 失败时保留旧文件。
...
- legacy Dexie `settings` 导入和运行时 mirror 已纳入 Storage v2 `settings` 表的 `dexie-settings` scope，覆盖用户头像、provider logo、pinned models、翻译偏好、MCP provider 服务列表等仍散落在 IndexedDB 的配置；Dexie settings hook 现在默认立即 flush，翻译页、迷你翻译、划词翻译、头像 / provider logo / pinned models、MCP provider 服务列表等用户设置写入也会通过统一 `putSettingAndFlush` 显式等待一次 mirror flush，底层 hook 仍作为兜底，确保直接 `db.settings.put/delete` 的写入也会自动落到 Storage v2。头像和 provider logo 删除会先 strict flush `null` 标记再删除 legacy Dexie 行；如果 Storage v2 临时不可写，Dexie settings mirror 会保留待同步 key 并延迟重试，避免立即重试形成紧循环。quick phrases、自定义翻译语言、翻译历史、知识库笔记这几张 Dexie 辅助表会以 `dexie.table.<table>.<id>` 行级记录进入 `dexie-table:<table>` scope，新增、更新和删除都会在关键用户操作后立即 flush；快捷短语、自定义翻译语言和翻译历史的删除会先 strict flush Storage v2 tombstone 再删除 legacy Dexie 行，清空翻译历史前也会先 read-through 补齐 Storage v2-only 历史行再批量写 tombstone，避免路径变化或崩溃后旧辅助数据复活；快捷短语删除后的剩余短语排序重排也会同步写回 Storage v2，避免恢复后顺序回退；Storage v2 read-through 会把缺失的 active 行投影回 Dexie，值为 `null` 的记录只作为删除标记参与防复活判断，不会被投影成 legacy 行；prune 已经是 `null` 的删除标记时不会重复写入，避免无意义递增版本和制造同步噪音。
...
- 工作台 HTML artifact 的实体文件会随 `Data/Workbench` 一起进入 Storage v2 backup / restore；从 Storage v2 读取或投影 workbench shortcut 时，会把 HTML artifact 的 `sourcePath` / `file://` URL 重写到当前 active data root 下的 `Workbench` 文件，避免跨路径恢复后快捷方式仍指向旧机器或旧产品名目录。
- 自定义小程序 / 工作台 HTML artifact 入口仍兼容读取 `Data/Files/custom-minapps.json`，但文件不存在时会先从 Storage v2 的 `minapps.custom` 设置恢复，写入和旧文件读取都会自愈镜像到 Storage v2，避免产品名、data root 迁移或只做 Storage v2 备份后丢失用户安装的小程序。
...
- Renderer 已加入普通对话 -> Storage v2 mirror：普通聊天消息、消息块等高频写入会按 topic 防抖镜像到 `conversations` / `messages` / `message_blocks`；topic 新增、标题/置顶/排序/所属 assistant 等低频元信息变更会立即 flush 对应 topic mirror，手动新建 topic、删除最后一个 topic 时创建替换 topic、分支会话克隆完成，以及语言切换时重命名默认 topic 的 UI 调用会等待这次 flush 完成后再继续，旧 Redux migrate 修正 message `assistantId` 后也会 flush 迁移后的 topic mirror；清空/删除消息和删除消息块会在操作返回前以 destructive strict flush 检查 mirror，降低崩溃窗口内漏写 tombstone 的风险，并在 Storage v2 暂不可写时向调用链暴露失败；这些操作会先完成 conversation tombstone，再清理关联附件文件，避免恢复出指向已 tombstone 文件的消息块；消息删除、消息块移除、重新生成/重发回复时的旧 block 清理、topic 清空和文件页删除附件引用，会先把最终会话快照写入 Storage v2，再更新 legacy Dexie / Redux UI，避免旧 IndexedDB 先删除成功但 Storage v2 尚未记录 tombstone 时崩溃后复活旧消息或旧 block。非破坏性 topic mirror 在 prune 前会先用非 prune conversation sync 合并当前 Dexie topic，再从 Storage v2 回填 Dexie 后执行最终 prune，避免用户在路径变化后直接发送/编辑消息时，用局部 Dexie cache 覆盖 Storage v2 中已有历史；删除、清空和移除 block 会标记为 destructive flush，跳过预回填，让 prune 正常写 tombstone。写入 Storage v2 失败、Storage v2 IPC 暂不可用或构建 Dexie topic 快照时临时读失败，都会保留待同步 topic 并按 destructive 标记重试，避免临时故障后直接丢掉 mirror 任务；mirror 只在 Dexie 明确存在该 topic cache 时写回，避免 Storage v2 read-through 尚未 seed Dexie 时把 Redux 里的空 topic 误当作清空会话，并只同步当前对话引用到的文件元信息。
- Renderer 已加入普通对话 / 文件显式删除 tombstone：删除 topic 或文件时会写入 Storage v2 软删除，避免恢复后复活；单个 topic 删除、清空助手全部 topic、删除助手这类破坏性入口会先等待 legacy runtime 删除和 Storage v2 tombstone 路径完成，再更新助手列表 Redux runtime cache，`assistants` reducer 不再直接触发异步删除副作用；topic 删除和文件删除都会先确认 Storage v2 tombstone API 可用并写入成功，再删除 legacy IndexedDB metadata 和本地文件，Storage v2 delete IPC 不可用时也会中止删除，批量文件删除会在任一 tombstone 失败时向上抛错，降低 UI 已删除但权威库尚未落盘的崩溃窗口。
...
- Renderer 已加入文件元信息 -> Storage v2 mirror：附件新增、更新、引用计数变化会通过 `file.upsert` 同步到 blob/file tables；如果 FileManager 的直接 `file.upsert` 失败，或 Storage v2 IPC 暂不可用，会把对应文件放回 file mirror 队列并立即 flush / 延迟重试 / 保留待重试任务，避免附件 metadata 只停留在 legacy Dexie；待同步文件在 flush 前已从 Dexie 消失时会补写 file tombstone，避免恢复后复活。
...
- Renderer 已加入 Dexie settings read-through：用户头像/自定义 provider logo、pinned models、mention models 面板、MCP provider 服务列表、翻译页设置、迷你翻译和划词翻译读取 legacy `db.settings` 为空时，会按 `dexie.settings.<id>` 从 Storage v2 恢复；值为 `null` 的记录仍视为删除标记，不会重新写回 legacy。头像等显式删除会立即 flush 删除标记，避免删除后的读穿透把旧配置恢复回来；如果 Storage v2 临时不可写或 IPC 暂不可用，Dexie settings mirror 会保留待同步 key 并延迟重试。MCP provider token 因服务端 401/403 被清除时会等待 durable localStorage strict flush 成功，避免旧 token 从 Storage v2 恢复回来。
- Renderer 已加入 Dexie 辅助表 read-through：快速短语、自定义翻译语言和翻译历史在列表读取时会从 Storage v2 对应 `dexie-table:*` scope 补齐 legacy 表中缺失的 active 行，即使 legacy 表并非空表也不会漏掉 Storage v2 中已有的用户资产；知识库笔记按条读取、编辑、迁移或列表渲染时如果 legacy 行缺失，也会按 `dexie.table.knowledge_notes.<id>` 从 Storage v2 单条恢复，降低 userData 路径变化或旧库局部缺行导致用户资产短暂消失的概率。快捷短语、翻译语言、翻译历史和知识库笔记的新增、更新、删除会立即 flush 对应行或删除标记，Storage v2 写入失败或 IPC 暂不可用时会保留待同步行并延迟重试；新增自定义翻译语言前也会先恢复 Storage v2 中已有语言再判重，避免 legacy 只有部分数据时写出重复语言。
- 主进程全局 MCP memory 图谱已加入 Storage v2 secret-vault 镜像：`memory.json` 会优先从 `mcp.memory.graph` 对应的加密 secret 恢复，legacy 文件缺失或落在旧 `userData` 路径时不会丢失记忆；每次新增、删除或更新实体/关系/观察记录都会先写入 Storage v2，再写回 legacy `memory.json`，旧文件读到的图谱也会自动补镜像到 Storage v2。
- Renderer / Main 已加入 Pi agent -> Storage v2 mirror：agent session 消息、agent/session/task/channel 等本地 IPC、本地 HTTP API、定时任务、claw MCP 工具和外部 channel 写操作进入 `agents.db` 后会导入 Storage v2，消息 exchange 持久化和 headless channel/scheduler exchange 完成后会立即 flush agent mirror，失败或 Storage v2 IPC 暂不可用时保留待重试任务并安排延迟重试；agent 创建/更新/排序、session 创建/更新/排序、task 创建/更新/运行日志、channel 创建/更新，以及 skill 安装/更新/启停都会在成功写入 legacy 后立即 flush agent mirror；agent mirror 每次执行 prune 导入前会先用非 prune seed 合并当前 legacy，再从 Storage v2 投影回 `agents.db`，避免用户在路径变化后第一步直接创建/编辑 agent 时，用不完整 legacy runtime prune 掉 Storage v2 中已有的 agent/session/task/channel；agent/session/message/task/channel 删除会先确认目标存在并写 Storage v2 direct tombstone，其中 session 与对应 agent conversation 在同一事务内 tombstone，再删除 legacy runtime 并要求 strict mirror 成功，skill 卸载也会先写 Storage v2 skill tombstone，再删除全局 skill 目录和 legacy row 并 strict flush；Renderer agent session 消息单删、批删和清空会调用主进程 IPC，按 `agents.db` 行 id 先 tombstone Storage v2 agent conversation message，再删除 legacy session_messages，成功后才允许 Redux UI 移除；若 tombstone 写入失败会保留 legacy 行不动，降低删除后崩溃或 Storage v2 临时不可写导致旧数据复活的风险；agent 删除现在会在 legacy `agents.db` 保留删除墓碑并清理 sessions/tasks/agent_skills、解绑 channel，普通用户创建的 agent 和内置 agent 都不会因为硬删丢掉本地删除证据；手动 legacy 导入和完整迁移仍默认保留 snapshot。Agent list/get/session/task/channel/message-history read 在 legacy runtime 为空、缺失或 agent 列表只包含部分可见 agent 时，会先用非 prune 模式把选中的 legacy data root 中仍存在的 `agents.db` 合并进 Storage v2，再串行投影 Storage v2 回当前 `agents.db` 后重读，避免投影重建时覆盖尚未镜像的 agent/session/task/channel 行；task/channel create 和 agent message persist 会先尝试恢复对应 runtime，再执行旧库写入；内置 agent 启动初始化前也会先尝试从 Storage v2 恢复，并且 legacy 里已有删除墓碑的 agent 不会被旧 Storage v2 数据复活；该 read-through / write-before-recovery 已覆盖 IPC、本地 HTTP API、调度器任务扫描/手动运行、Pi runtime 历史上下文加载、channel manager 启动/同步和 channel message session 解析，用于覆盖产品名或 userData 路径变化后的本地恢复场景。
